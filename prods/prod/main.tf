...
module "container_registry" {
  ...
  log_analytics_workspace_id   = module.log_analytics_workspace.id
  log_analytics_retention_days = var.log_analytics_retention_days
  random                       = random_string.storage_account_suffix.result
  env                          = var.env 
}
...
module "aks_cluster" {
  ...
  vnet_appgwsubnet_id                      = module.aks_network.subnet_ids[var.appgw_subnet_name]
...
resource "azurerm_role_assignment" "appgw_contributor" {
  scope                = module.aks_cluster.appgw_id
  role_definition_name = "Contributor"
  principal_id         = module.aks_cluster.appgw_principal_id
  skip_service_principal_aad_check = true
}
...
module "virtual_machine" {
  ...
  log_analytics_retention_days        = var.log_analytics_retention_days
  script_storage_account_name         = "${local.storage_account_prefix}${random_string.storage_account_suffix.result}"
  script_storage_account_key          = "${local.storage_account_prefix}${random_string.storage_account_suffix.result}"
...
module "postgresql_private_dns_zone" {
  source                       = "../../modules/private_dns_zone"
...
module "postgresql" {
  ...
  postgres_subnet_id  = module.aks_network.subnet_ids[var.postgres_subnet_name]
  key_vault_id        = module.key_vault.id
  access_id           = module.key_vault.access_id
  log_analytics_workspace_id   = module.log_analytics_workspace.id
  log_analytics_retention_days = var.log_analytics_retention_days
}
...
module "postgresql_private_endpoint" {
  source                         = "../../modules/private_endpoint"
  name                           = "${title(module.postgresql.name)}PrivateEndpoint"
  location                       = var.location
  resource_group_name            = azurerm_resource_group.rg.name
  subnet_id                      = module.aks_network.subnet_ids[var.vm_subnet_name]
  tags                           = var.tags
  ...
  private_dns_zone_group_ids     = [module.postgresql_private_dns_zone.id]
}
...
resource "azurerm_key_vault_secret" "ssh_public_key" {
  ...
  depends_on = [module.key_vault.access_id, time_sleep.wait_30_seconds]
}
